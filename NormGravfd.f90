...
!============================================================================
!
      SUBROUTINE cstonorm (maxn,djn,GRS,HC,HS)
!MS$ATTRIBUTES DLLEXPORT::CSTONORM
C THE EVEN DEGREE ZONAL COEFFICIENTS of NORMAL GRAVITY FIELD WERE 
C substracted from HC, HS
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8::HC((maxn+2)**2),HS((maxn+2)**2),djn(80)
	real*8::GRS(6)
	do 101 n=2,maxn,2
	HC(n*(n+1)/2)=HC(n*(n+1)/2)+djn(n)/dsqrt(2.d0*n+1.d0)
  101	continue
      RETURN
      END
